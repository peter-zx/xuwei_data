...
class FileScanner:
    ...
    def __init__(self, root_path: str):
        ...
        self.excluded_folders = {'__pycache__', '.git', 'node_modules', '.venv', 'venv', '.venv', 'node_modules'}
    ...
    def _render_tree(self, node: FileNode, level: int, parent_names: list = None) -> str:
        ...
        if node.is_folder:
            current_parents = parent_names + [node.name]
            current_parents_str = '|'.join(current_parents)
    ...
    def _render_file(self, node: FileNode, parent_folders: str = "") -> str:
        ...
        html += f'<input type="checkbox" data-path="{node.path}" data-folders="{parent_folders}" class="file-checkbox" style="width:18px; height:18px; cursor:pointer; margin-right:12px;">'
